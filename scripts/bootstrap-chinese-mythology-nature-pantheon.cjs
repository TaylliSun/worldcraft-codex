...
const { applyAncientCoreBatch } = require("./bootstrap-chinese-mythology-ancient-core.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-nature-pantheon-02.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-nature-pantheon-02-${timestampForFile(now)}.wcodex`
  );
...
function applyNaturePantheonBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyAncientCoreBatch(data, now);
  const foundation = buildMythologyFoundation(now, worldId);
  const ancientCore = buildAncientCoreBatch(now, worldId);
  const batch = buildNaturePantheonBatch(now, worldId);
  ...
  const managedIds = new Set([
    ...
    ...ancientCore.entities.map((item) => item.id),
...
function auditNaturePantheonBatch(data, worldId) {
  const foundation = buildMythologyFoundation("2000-01-01T00:00:00.000Z", worldId);
  const ancientCore = buildAncientCoreBatch("2000-01-01T00:00:00.000Z", worldId);
  const expected = buildNaturePantheonBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  const styleHits = entities
    .filter((item) => /古老而神秘|命运的齿轮|不为人知的秘密|最终揭开真相|值得注意的是|综上所述|规则复核|索引说明/u.test(`${item.summary}\n${item.content}`))
    .map((item) => item.title);
  ...
  const managedRelationIds = new Set([...ancientCore.relations, ...expected.relations].map((item) => item.id));
  const managedEventIds = new Set([...ancientCore.timelineEvents, ...expected.timelineEvents].map((item) => item.id));
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyNaturePantheonBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-nature-pantheon-02");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditNaturePantheonBatch(persisted, worldId);
  ...
  assert.deepEqual(audit.cumulativeManagedCounts, { entities: 108, relations: 148, timelineEvents: 42 });
...
module.exports = { applyNaturePantheonBatch, auditNaturePantheonBatch };
