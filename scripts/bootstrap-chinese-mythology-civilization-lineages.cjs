...
const { applyNaturePantheonBatch } = require("./bootstrap-chinese-mythology-nature-pantheon.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-civilization-lineages-03.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-civilization-lineages-03-${timestampForFile(now)}.wcodex`
  );
...
function applyCivilizationLineagesBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyNaturePantheonBatch(data, now);
  ...
  const batch = buildCivilizationLineagesBatch(now, worldId);
...
function auditCivilizationLineagesBatch(data, worldId) {
  ...
  const expected = buildCivilizationLineagesBatch(auditTime, worldId);
  ...
  const phaseOneThresholdIssues = [];
  if (cumulativeManagedCounts.entities < 120) phaseOneThresholdIssues.push("公开条目少于 120");
  if (cumulativeManagedCounts.relations < 300) phaseOneThresholdIssues.push("证据关系少于 300");
  if (cumulativeManagedCounts.timelineEvents < 50) phaseOneThresholdIssues.push("时间点少于 50");
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyCivilizationLineagesBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-civilization-lineages-03");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditCivilizationLineagesBatch(persisted, worldId);
  ...
  assert.deepEqual(audit.cumulativeManagedCounts, { entities: 171, relations: 336, timelineEvents: 64 });
  ...
  const report = {
    ...
    phase: "中国上古神话史 · 阶段 1 · 圣王谱系与文明技艺收官批次",
...
module.exports = { applyCivilizationLineagesBatch, auditCivilizationLineagesBatch };
