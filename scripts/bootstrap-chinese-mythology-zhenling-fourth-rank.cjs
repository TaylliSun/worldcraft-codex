...
const { applyZhenlingThirdRankBatch } = require("./bootstrap-chinese-mythology-zhenling-third-rank.cjs");
const { zhenlingSourceId } = require("./chinese-mythology-zhenling-first-rank-data.cjs");
const { BATCH_LABEL, buildZhenlingFourthRankBatch, zhenlingFourthEntityId } = require("./chinese-mythology-zhenling-fourth-rank-data.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-zhenling-fourth-rank-15.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-zhenling-fourth-rank-15-${now.replace(/[:.]/g, "-")}.wcodex`);
...
function applyZhenlingFourthRankBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyZhenlingThirdRankBatch(data, now);
...
function auditBatch(data, worldId) {
  const expected = buildZhenlingFourthRankBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    ...
    preservedBoundaries: {
      ...
      structureHeadings: expected.institutions.filter((item) => /十五玉女|四将军|十五使者|太清五帝|三位女真/u.test(item.title)).length,
      wangwuLocations: expected.locations.filter((item) => item.title.includes("王屋山")).length,
      disputedIdentityRelations: relations.filter((item) => item.kind === "disputed").length,
      sourceEntryId: zhenlingSourceId(worldId),
      tierEntryId: zhenlingFourthEntityId("fourth-rank-taiqing", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyZhenlingFourthRankBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-zhenling-fourth-rank-15");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.counts, { entities: 175, figures: 157, institutions: 17, locations: 1, sources: 0, catalogPositions: 174, reusedPositions: 6, collectiveGroups: 11, relations: 417, sourceRelations: 181, timelineEvents: 8 });
  assert.deepEqual(batchAudit.preservedBoundaries, { sectionCounts: { "中位": 2, "左位": 63, "右位": 109 }, structureHeadings: 5, wangwuLocations: 1, disputedIdentityRelations: 4, sourceEntryId: zhenlingSourceId(worldId), tierEntryId: zhenlingFourthEntityId("fourth-rank-taiqing", worldId) });
...
module.exports = { applyZhenlingFourthRankBatch, auditBatch };
