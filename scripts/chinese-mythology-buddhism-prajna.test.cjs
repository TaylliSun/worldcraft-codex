...
const batch = buildBuddhismPrajnaBatch(now, foundation.world.id);

const idsAreUnique = (items) => new Set(items.map((item) => item.id)).size === items.length;
const plainText = (value) => String(value || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();

assert.equal(batch.entities.length, 76);
assert.equal(batch.figures.length, 8);
assert.equal(batch.concepts.length, 16);
assert.equal(batch.sources.length, 52);
assert.equal(batch.coreSources.length, 20);
assert.equal(batch.sectionSources.length, 32);
assert.equal(batch.relations.length, 217);
assert.equal(batch.timelineEvents.length, 29);
...
for (const relation of batch.relations) {
  ...
  assert.ok(relationKinds.has(relation.kind), `${relation.id}: kind ${relation.kind}`);
  assert.ok(evidenceTypes.has(relation.evidenceType), `${relation.id}: evidence ${relation.evidenceType}`);
...
assert.equal(sourceRelations.length, 24);
...
const sections = batch.sources.filter((item) => item.tags.includes("金刚经三十二分"));
assert.equal(sections.length, 32);
assert.equal(sections.every((item) => item.content.includes("后世三十二分导航")), true);
assert.equal(batch.relations.filter((item) => item.label.startsWith("后世导航第 ")).length, 32);
assert.equal(batch.relations.filter((item) => item.label === "三十二分流通本下一分").length, 31);
assert.equal(batch.relations.filter((item) => item.label === "本分核心阅读概念").length, 32);

assert.deepEqual(
  {
    entities: transmission.entities.length + devotion.entities.length + schools.entities.length + batch.entities.length,
    relations: transmission.relations.length + devotion.relations.length + schools.relations.length + batch.relations.length,
    timelineEvents: transmission.timelineEvents.length + devotion.timelineEvents.length + schools.timelineEvents.length + batch.timelineEvents.length,
    sourceEntries: transmission.sources.length + devotion.sources.length + schools.sources.length + batch.sources.length
  },
  { entities: 256, relations: 717, timelineEvents: 89, sourceEntries: 81 }
);

console.log("Chinese mythology Buddhism Prajna checks passed: 76 entries, 217 sourced relations, 29 events; Buddhist direct totals 256/717/89 with 81 scripture and commentary entries.");
