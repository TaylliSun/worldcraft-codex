...
const { applyDingjiaGuardiansBatch } = require("./bootstrap-chinese-mythology-dingjia-guardians.cjs");
const {
  BATCH_LABEL,
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-prajna-22.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-buddhism-prajna-22-${now.replace(/[:.]/g, "-")}.wcodex`
  );
...
function applyBuddhismPrajnaBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyDingjiaGuardiansBatch(data, now);
  const batch = buildBuddhismPrajnaBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildBuddhismPrajnaBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    counts: {
      ...
      concepts: entities.filter((item) => item.templateId?.endsWith(":institution-ritual")).length,
      sources: entities.filter((item) => item.templateId?.endsWith(":source-text")).length,
      coreSources: entities.filter((item) => expected.coreSources.includes(item.id)).length,
      sectionSources: entities.filter((item) => expected.sectionSources.includes(item.id)).length,
      relations: relations.length,
      sourceRelations: relations.filter((item) => item.id.includes(":source-")).length,
      sectionContains: relations.filter((item) => item.label?.startsWith("后世导航第 ")).length,
      sectionSequence: relations.filter((item) => item.label === "三十二分流通本下一分").length,
      sectionConcepts: relations.filter((item) => item.label === "本分核心阅读概念").length,
    ...
    preservedBoundaries: {
      diamondKumarajivaId: prajnaSourceId("diamond-kumarajiva", worldId),
      heartXuanzangId: prajnaSourceId("heart-xuanzang", worldId),
      firstSectionId: prajnaSourceId("diamond-section-01", worldId),
      lastSectionId: prajnaSourceId("diamond-section-32", worldId),
      sixTranslationsId: prajnaEntityId("six-translations-comparison", worldId),
      sectionSystemId: prajnaEntityId("thirty-two-sections", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyBuddhismPrajnaBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-prajna-22");
  } finally {
    store.close();
  }
...
module.exports = { applyBuddhismPrajnaBatch, auditBatch };
