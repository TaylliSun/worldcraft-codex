...
const reportPath = path.join(validationDir, "chinese-mythology-daoism-lineages-ui.json");
const wikiScreenshotPath = path.join(validationDir, "chinese-mythology-daoism-lineages-wiki.png");
const relationsScreenshotPath = path.join(validationDir, "chinese-mythology-daoism-lineages-relations.png");
const timelineScreenshotPath = path.join(validationDir, "chinese-mythology-daoism-lineages-timeline.png");
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const batchMarker = ":mythology:daoism-lineages:";
    ...
    assert.deepEqual(dataAudit.batch, {
      ...
      relations: 180,
      sourceRelations: 43,
      relationEvidenceComplete: 180,
      events: 20
    });
    assert.deepEqual(dataAudit.cumulativeManaged, { entities: 351, relations: 936, events: 134 });
    ...
    await page.locator(".wiki-stat-band").getByText("351 篇文章", { exact: true }).waitFor();
    await page.locator(".wiki-stat-band").getByText("134 个历史节点", { exact: true }).waitFor();

    const wikiSearch = page.getByLabel("搜索世界 Wiki");
    await wikiSearch.fill("张宗演");
    await (await exactWikiResult(page, "张宗演（三十六代天师）")).waitFor({ state: "visible", timeout: 60000 });
    await wikiSearch.fill("张留孙");
    const liusunResult = await exactWikiResult(page, "张留孙（玄教大宗师）");
    await liusunResult.waitFor({ state: "visible", timeout: 60000 });
    await liusunResult.click();
    await page.locator(".wiki-article h1").getByText("张留孙（玄教大宗师）", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByRole("heading", { name: "生平与位置", exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByText("张留孙随张宗演进京", { exact: false }).waitFor();
    await page.locator(".wiki-rich-content").getByText("不受天师号", { exact: false }).waitFor();
    ...
    await page.locator(".relation-list-panel").getByText("936 条符合筛选", { exact: true }).waitFor();
    await page.getByLabel("搜索关系").fill("玄教");
    const xuanjiaoRelation = page.locator(".explicit-relation-card").filter({
      hasText: "源出龙虎山网络而制度有别"
    });
    await xuanjiaoRelation.waitFor({ state: "visible", timeout: 60000 });
    await xuanjiaoRelation.click();
    ...
    assert.match(await page.getByLabel("关系原典出处").inputValue(), /元史/u);
    assert.match(await page.getByLabel("关系适用年代").inputValue(), /元代/u);
    ...
    assert.match((await trackButtons.filter({ hasText: "神话叙事" }).textContent()) || "", /52 项/u);
    assert.match((await trackButtons.filter({ hasText: "文献证据" }).textContent()) || "", /47 项/u);
    assert.match((await trackButtons.filter({ hasText: "宗教与礼制制度" }).textContent()) || "", /23 项/u);
    assert.match((await trackButtons.filter({ hasText: "信仰演变与合流" }).textContent()) || "", /12 项/u);
    await page.locator(".timeline-track-board").getByText("《净明忠孝全书》泰定四年序成", { exact: true }).waitFor();
    await page.locator(".timeline-track-board").getByText("元世祖召张宗演主领江南道教", { exact: true }).waitFor();
    ...
    const report = {
      ...
      dataAudit,
      ui: {
        wikiCountsVisible: { articles: 351, timelineEvents: 134 },
        disambiguationSearchVisible: ["张宗演（三十六代天师）", "张留孙（玄教大宗师）"],
        openedArticle: "张留孙（玄教大宗师）",
        evidenceRelationVisible: "源出龙虎山网络而制度有别",
        timelineCountsVisible: { mythic: 52, textual: 47, institutional: 23, evolution: 12 }
      },
