...
const { applyZhenlingSixthRankBatch, auditBatch } = require("./bootstrap-chinese-mythology-zhenling-sixth-rank.cjs");
...
const relationMarker = ":mythology:zhenling-sixth-rank:";
...
try {
  ...
  const first = applyZhenlingSixthRankBatch(data, "2026-07-19T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyZhenlingSixthRankBatch(data, "2026-07-19T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 191, relations: 626, timelineEvents: 8 });
  ...
  assert.equal(fullAudit.ok, true, `第六阶预演后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
