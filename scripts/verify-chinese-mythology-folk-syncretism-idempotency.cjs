...
const {
  applyFolkSyncretismBatch,
  auditFolkSyncretismBatch
} = require("./bootstrap-chinese-mythology-folk-syncretism.cjs");
...
const marker = ":mythology:folk-syncretism:";
...
try {
  ...
  const first = applyFolkSyncretismBatch(data, now);
  const firstCounts = counts(data, first.worldId);
  const second = applyFolkSyncretismBatch(data, now);
  ...
  const audit = auditFolkSyncretismBatch(data, second.worldId);
