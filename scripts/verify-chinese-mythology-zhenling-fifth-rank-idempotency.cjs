...
const { applyZhenlingFifthRankBatch, auditBatch } = require("./bootstrap-chinese-mythology-zhenling-fifth-rank.cjs");
...
const relationMarker = ":mythology:zhenling-fifth-rank:";
...
try {
  ...
  const first = applyZhenlingFifthRankBatch(data, "2026-07-19T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyZhenlingFifthRankBatch(data, "2026-07-19T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 47, relations: 136, timelineEvents: 6 });
  ...
  assert.equal(fullAudit.ok, true, `第五阶预演后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
