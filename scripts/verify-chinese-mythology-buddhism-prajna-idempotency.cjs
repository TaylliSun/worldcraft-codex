...
const { applyBuddhismPrajnaBatch, auditBatch } = require("./bootstrap-chinese-mythology-buddhism-prajna.cjs");
...
try {
  ...
  const first = applyBuddhismPrajnaBatch(data, "2026-07-20T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyBuddhismPrajnaBatch(data, "2026-07-20T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 76, relations: 217, timelineEvents: 29 });
  ...
  assert.equal(fullAudit.ok, true, `般若经典批次重复导入后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
