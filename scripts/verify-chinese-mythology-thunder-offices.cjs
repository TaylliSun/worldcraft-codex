...
const reportPath = path.join(validationDir, "chinese-mythology-thunder-offices-ui.json");
const wikiScreenshotPath = path.join(validationDir, "chinese-mythology-thunder-offices-wiki.png");
const relationsScreenshotPath = path.join(validationDir, "chinese-mythology-thunder-offices-relations.png");
const timelineScreenshotPath = path.join(validationDir, "chinese-mythology-thunder-offices-timeline.png");
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const marker = ":mythology:daoism-thunder-offices:";
    ...
    assert.deepEqual(dataAudit.batch, { entities: 82, figures: 64, institutions: 18, relations: 256, sourceRelations: 82, events: 18, ritualRecordRelations: 1 });
    assert.deepEqual(dataAudit.cumulative, { entities: 1457, relations: 4148, events: 327 });
    assert.deepEqual(dataAudit.semantics, { sourceRelations: 1241, teacherRelations: 115, mutualRelations: 52, yearPrecisionEvents: 8 });
    assert.deepEqual(dataAudit.tracks, { "神话叙事": 70, "文献证据": 101, "宗教与礼制制度": 98, "信仰演变与合流": 58 });
    ...
    await page.locator(".wiki-stat-band").getByText("1457 篇文章", { exact: true }).waitFor();
    await page.locator(".wiki-stat-band").getByText("327 个历史节点", { exact: true }).waitFor();
    await page.getByLabel("搜索世界 Wiki").fill("雷师皓翁");
    const result = exactWikiResult(page, "雷师皓翁");
    await result.waitFor({ state: "visible", timeout: 60000 });
    await result.click();
    await page.locator(".wiki-article h1").getByText("雷师皓翁", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content p").filter({ hasText: "《玉枢宝经》说法对象中亦见雷师皓翁，本批只保存卷五十六神位。" }).waitFor();
    await page.locator(".wiki-rich-content").getByText("体系边界", { exact: true }).waitFor();
    await page.screenshot({ path: wikiScreenshotPath, fullPage: false });

    await openMoreTool(page, "关系图");
    await page.locator(".relation-list-panel").getByText("4148 条符合筛选", { exact: true }).waitFor();
    await page.getByLabel("搜索关系").fill("玉枢雷霆高位权威");
    const relationCard = page.locator(".explicit-relation-card").filter({ hasText: "玉枢雷霆高位权威与卷五十六神位表并读" }).first();
    await relationCard.waitFor({ state: "visible", timeout: 60000 });
    await relationCard.click();
    assert.equal(await page.getByLabel("关系证据类型").inputValue(), "ritual-record");
    ...
    assert.match((await trackButtons.filter({ hasText: "文献证据" }).textContent()) || "", /101 项/u);
    assert.match((await trackButtons.filter({ hasText: "宗教与礼制制度" }).textContent()) || "", /98 项/u);
    await trackButtons.filter({ hasText: "文献证据" }).click();
    const event = page.locator(".timeline-track-board").getByText("雷师皓翁与五雷皓翁在神位表中分列", { exact: true });
    await event.waitFor();
    ...
    const report = {
      ...
      ui: { wikiCountsVisible: { articles: 1457, timelineEvents: 327 }, openedArticle: "雷师皓翁", evidenceRelationVisible: "玉枢雷霆高位权威与卷五十六神位表并读", timelineEventVisible: "雷师皓翁与五雷皓翁在神位表中分列" },
