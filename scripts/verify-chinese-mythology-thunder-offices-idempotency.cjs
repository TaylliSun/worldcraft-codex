...
const { applyThunderOfficesBatch, auditBatch } = require("./bootstrap-chinese-mythology-thunder-offices.cjs");
...
const entityMarker = ":mythology:daoism-thunder-offices:";
...
try {
  ...
  const first = applyThunderOfficesBatch(data, "2026-07-19T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyThunderOfficesBatch(data, "2026-07-19T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 82, relations: 256, timelineEvents: 18 });
  ...
  assert.equal(fullAudit.ok, true, `雷部预演后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
