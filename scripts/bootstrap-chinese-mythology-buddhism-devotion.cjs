...
const { applyBuddhismTransmissionBatch } = require("./bootstrap-chinese-mythology-buddhism-transmission.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-devotion-08.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-buddhism-devotion-08-${timestampForFile(now)}.wcodex`
  );
...
function buildAllBatches(now, worldId) {
  return {
    ...
    devotion: buildBuddhismDevotionBatch(now, worldId)
  };
}

function applyBuddhismDevotionBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyBuddhismTransmissionBatch(data, now);
  const batches = buildAllBatches(now, worldId);
  const batch = batches.devotion;
...
function auditBuddhismDevotionBatch(data, worldId) {
  ...
  const managedRelationIds = new Set([
    ...
    ...batches.buddhismTransmission.relations,
    ...expected.relations
  ].map((item) => item.id));
  const managedEventIds = new Set([
    ...
    ...batches.buddhismTransmission.timelineEvents,
    ...expected.timelineEvents
  ].map((item) => item.id));
  const phase3EntityIds = new Set([
    ...batches.buddhismTransmission.entities,
    ...expected.entities
  ].map((item) => item.id));
  const phase3RelationIds = new Set([
    ...batches.buddhismTransmission.relations,
    ...expected.relations
  ].map((item) => item.id));
  const phase3EventIds = new Set([
    ...batches.buddhismTransmission.timelineEvents,
    ...expected.timelineEvents
  ].map((item) => item.id));
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyBuddhismDevotionBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-devotion-08");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditBuddhismDevotionBatch(persisted, worldId);
  ...
  const report = {
    ...
    phase: "中国上古神话史 · 阶段 3 · 菩萨道场、罗汉与护法天部第二批",
...
module.exports = { applyBuddhismDevotionBatch, auditBuddhismDevotionBatch };
