...
assert.equal(batch.figures.some((item) => item.title === "黄帝"), false);
assert.equal(batch.figures.some((item) => item.title === "孔子"), false);
assert.equal(batch.entities.some((item) => item.id === zhenlingThirdEntityId("third-rank-taiji", worldId)), true);
...
for (const figure of batch.figures) {
  ...
  assert.equal(figure.templateData.rankPosition, "第三阶 · 太极境");
}
...
assert.equal(sourceRelations.length, 87);
assert.equal(sourceRelations.every((item) => item.kind === "source" && item.targetEntityId === zhenlingSourceId(worldId)), true);
assert.equal(batch.relations.filter((item) => item.kind === "contains").length, 86);
assert.equal(batch.relations.filter((item) => item.kind === "teacher").length, 7);
assert.equal(batch.relations.filter((item) => item.kind === "family").length, 3);
...
assert.deepEqual(batch.timelineEvents.reduce((counts, item) => {
  ...
  return counts;
}, {}), { "textual-evidence": 2, "religious-institutions": 1, "cult-evolution": 3 });
...
}, { entities: 856, relations: 2422, timelineEvents: 278 });

console.log("Chinese mythology Zhenling third-rank checks passed: 86 source positions, 11 reused identities, 71 new figures, 4 collective groups, 76 entries, 189 relations, 6 events; cumulative totals 856/2422/278.");
