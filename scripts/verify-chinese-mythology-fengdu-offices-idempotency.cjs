...
const { applyFengduOfficesBatch, auditBatch } = require("./bootstrap-chinese-mythology-fengdu-offices.cjs");
...
const entityMarker = ":mythology:daoism-fengdu-offices:";
...
try {
  ...
  const first = applyFengduOfficesBatch(data, "2026-07-19T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyFengduOfficesBatch(data, "2026-07-19T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 94, relations: 290, timelineEvents: 44 });
  ...
  assert.equal(fullAudit.ok, true, `酆都预演后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
