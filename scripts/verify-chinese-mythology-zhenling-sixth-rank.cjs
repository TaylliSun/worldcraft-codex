...
const reportPath = path.join(validationDir, "chinese-mythology-zhenling-sixth-rank-ui.json");
const wikiScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-sixth-rank-wiki.png");
const relationsScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-sixth-rank-relations.png");
const timelineScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-sixth-rank-timeline.png");
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const entityMarker = ":mythology:zhenling-weiye:sixth-rank:";
      const marker = ":mythology:zhenling-sixth-rank:";
    ...
    assert.deepEqual(dataAudit.batch, { entities: 191, figures: 163, institutions: 13, locations: 15, relations: 626, sourceRelations: 197, events: 8, disputedRelations: 1, teacherRelations: 12 });
    assert.deepEqual(dataAudit.cumulative, { entities: 1269, relations: 3601, events: 300 });
    assert.deepEqual(dataAudit.semantics, { sourceRelations: 1047, teacherRelations: 115, mutualRelations: 52, yearPrecisionEvents: 8 });
    assert.deepEqual(dataAudit.tracks, { "神话叙事": 70, "文献证据": 94, "宗教与礼制制度": 79, "信仰演变与合流": 57 });
    ...
    await page.locator(".wiki-stat-band").getByText("1269 篇文章", { exact: true }).waitFor();
    await page.locator(".wiki-stat-band").getByText("300 个历史节点", { exact: true }).waitFor();
    await page.getByLabel("搜索世界 Wiki").fill("许迈");
    const result = exactWikiResult(page, "许迈");
    await result.waitFor({ state: "visible", timeout: 60000 });
    await result.click();
    await page.locator(".wiki-article h1").getByText("许迈", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByText("夹注称其字叔玄、小名晄，改名远游，并被东华署为地仙。", { exact: true }).waitFor();
    ...
    await page.locator(".relation-list-panel").getByText("3601 条符合筛选", { exact: true }).waitFor();
    await page.getByLabel("搜索关系").fill("第四阶与第六阶周君");
    const relationCard = page.locator(".explicit-relation-card").filter({ hasText: "第四阶与第六阶周君同一性待考" }).first();
    ...
    assert.match((await trackButtons.filter({ hasText: "文献证据" }).textContent()) || "", /94 项/u);
    assert.match((await trackButtons.filter({ hasText: "宗教与礼制制度" }).textContent()) || "", /79 项/u);
    assert.match((await trackButtons.filter({ hasText: "信仰演变与合流" }).textContent()) || "", /57 项/u);
    await trackButtons.filter({ hasText: "信仰演变与合流" }).click();
    const event = page.locator(".timeline-track-board").getByText("第六阶夹注展开山岳与洞室修真地理网", { exact: true });
    await event.waitFor();
    ...
    const report = {
      ...
      ui: { wikiCountsVisible: { articles: 1269, timelineEvents: 300 }, openedArticle: "许迈", evidenceRelationVisible: "第四阶与第六阶周君同一性待考", timelineEventVisible: "第六阶夹注展开山岳与洞室修真地理网" },
