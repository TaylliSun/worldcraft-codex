...
const batch = buildDaoismEarlyBatch(now, foundation.world.id);
...
assert.equal(batch.entities.length, 70);
assert.equal(batch.figures.length, 31);
assert.equal(batch.institutions.length, 12);
assert.equal(batch.locations.length, 8);
assert.equal(batch.sources.length, 19);
assert.equal(batch.relations.length, 200);
assert.equal(batch.timelineEvents.length, 28);
...
for (const relation of batch.relations) {
  ...
  assert.ok(evidenceTypes.has(relation.evidenceType), `${relation.id}: evidence type`);
...
const traditional142 = batch.timelineEvents.find((item) => item.id.endsWith(":laojun-grants-daoling"));
assert.ok(traditional142);
assert.equal(traditional142.displayDate, "天师道传统纪年：汉安元年");
assert.doesNotMatch(`${traditional142.startValue}${traditional142.endValue}`, /142/);
...
for (const title of [
  ...
  assert.ok(batch.entities.some((item) => item.title === title), `${title}: disambiguation entry`);
}
...
const variantRelation = batch.relations.find((item) => item.label === "传写相近但并非一人");
assert.equal(variantRelation.evidenceType, "textual-variant");
assert.equal(variantRelation.confidence, "disputed");

assert.deepEqual(
  {
    entities: allEntities.length,
    relations: ancientCore.relations.length + naturePantheon.relations.length + civilizationLineages.relations.length + batch.relations.length,
    timelineEvents: ancientCore.timelineEvents.length + naturePantheon.timelineEvents.length + civilizationLineages.timelineEvents.length + batch.timelineEvents.length
  },
  { entities: 241, relations: 536, timelineEvents: 92 }
);

const appSource = require("node:fs").readFileSync(require("node:path").join(__dirname, "..", "app", "page.tsx"), "utf8");
assert.match(appSource, /"textual-variant": "传本异文"/u);

console.log("Chinese mythology early Daoism checks passed: 70 entries, 200 sourced relations, 28 four-track events; cumulative totals 241/536/92.");
