...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-han-people-ui.json");
const screenshots = {
  wiki: path.join(validationDir, "chinese-mythology-buddhism-han-people-wiki.png"),
  relations: path.join(validationDir, "chinese-mythology-buddhism-han-people-relations.png"),
  timeline: path.join(validationDir, "chinese-mythology-buddhism-han-people-timeline.png")
};
const worldName = "中国上古神话史";
const articleTitle = "道安";
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const marker = ":mythology:buddhism-han-people:";
    ...
    assert.deepEqual(dataAudit.cumulative, { entities: 2208, relations: 6673, events: 573 });
    ...
    await page.locator(".wiki-stat-band").getByText("2208 篇文章", { exact: true }).waitFor();
    await page.getByLabel("搜索世界 Wiki").fill(articleTitle);
    const result = page.locator(".wiki-search-group button").filter({
      has: page.locator("strong").getByText(articleTitle, { exact: true })
    });
    ...
    await page.locator(".wiki-rich-content").getByText("东晋僧团领袖", { exact: false }).first().waitFor();
    ...
    await page.locator(".relation-list-panel").getByText("6673 条符合筛选", { exact: true }).waitFor();
    ...
    await relationSearch.fill("译本扩展推动义学分化");
    const relation = page.locator(".explicit-relation-card").filter({ hasText: "译本扩展推动义学分化" }).first();
    ...
    assert.match((await track.textContent()) || "", /205 项/u);
    await track.click();
    const timelineEvent = page.locator(".timeline-track-board").getByText("道安整理经录并推动译经规范", { exact: true });
    ...
    const report = {
      ...
      ui: {
        ...
        relationVisible: "译本扩展推动义学分化",
        timelineEventVisible: "道安整理经录并推动译经规范"
      },
