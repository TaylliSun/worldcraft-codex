...
const reportPath = path.join(validationDir, "chinese-mythology-zhenling-second-rank-ui.json");
const wikiScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-second-rank-wiki.png");
const relationsScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-second-rank-relations.png");
const timelineScreenshotPath = path.join(validationDir, "chinese-mythology-zhenling-second-rank-timeline.png");
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const managedEntities = data.entities.filter((item) => item.worldId === world.id && item.id.includes(":mythology:"));
      const managedRelations = data.relations.filter((item) => item.worldId === world.id && item.id.includes(":mythology:"));
      const managedEvents = data.timelineEvents.filter((item) => item.worldId === world.id && item.id.includes(":mythology:"));
      return {
        worldId: world.id,
        batch: {
          ...
          disputedRedPineRelations: batchRelations.filter((item) => item.kind === "disputed" && item.id.endsWith(":chisongzi-yushi-disputed")).length
        },
        cumulativeManaged: {
          entities: managedEntities.length,
    ...
    assert.deepEqual(dataAudit.batch, {
      ...
      sourceRelations: 106,
      events: 6,
      disputedRedPineRelations: 1
    });
    assert.deepEqual(dataAudit.cumulativeManaged, { entities: 780, relations: 2233, events: 272 });
    ...
    await page.locator(".wiki-stat-band").getByText("780 篇文章", { exact: true }).waitFor();
    await page.locator(".wiki-stat-band").getByText("272 个历史节点", { exact: true }).waitFor();

    const wikiSearch = page.getByLabel("搜索世界 Wiki");
    await wikiSearch.fill("王方平");
    const result = exactWikiResult(page, "王方平");
    await result.waitFor({ state: "visible", timeout: 60000 });
    await result.click();
    await page.locator(".wiki-article h1").getByText("王方平", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByText("原注称其姓王，讳远，字方平；为紫阳君弟子、司命茅君之师。", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByText("身份处理", { exact: true }).waitFor();
    ...
    await page.locator(".relation-list-panel").getByText("2233 条符合筛选", { exact: true }).waitFor();
    await page.getByLabel("搜索关系").fill("赤松子");
    const relationCard = page.locator(".explicit-relation-card").filter({
      hasText: "赤松子与雨师传统的同一性有层次差异"
    }).first();
    ...
    assert.match((await trackButtons.filter({ hasText: "文献证据" }).textContent()) || "", /86 项/u);
    assert.match((await trackButtons.filter({ hasText: "宗教与礼制制度" }).textContent()) || "", /70 项/u);
    assert.match((await trackButtons.filter({ hasText: "信仰演变与合流" }).textContent()) || "", /46 项/u);
    await trackButtons.filter({ hasText: "文献证据" }).click();
    await page.locator(".timeline-track-board").getByText("第二中位形成上清境名位序列", { exact: true }).waitFor();
    ...
    const report = {
      ...
      dataAudit,
      ui: {
        wikiCountsVisible: { articles: 780, timelineEvents: 272 },
        openedArticle: "王方平",
        evidenceRelationVisible: "赤松子与雨师传统的同一性有层次差异",
        timelineEventVisible: "第二中位形成上清境名位序列"
      },
