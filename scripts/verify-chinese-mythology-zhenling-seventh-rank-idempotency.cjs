...
const { applyZhenlingSeventhRankBatch, auditBatch } = require("./bootstrap-chinese-mythology-zhenling-seventh-rank.cjs");
...
const relationMarker = ":mythology:zhenling-seventh-rank:";
...
try {
  ...
  const first = applyZhenlingSeventhRankBatch(data, "2026-07-19T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyZhenlingSeventhRankBatch(data, "2026-07-19T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 106, relations: 291, timelineEvents: 9 });
  ...
  assert.equal(fullAudit.ok, true, `第七阶预演后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
