...
const { applyBuddhismCanonBatch, auditBatch } = require("./bootstrap-chinese-mythology-buddhism-canon.cjs");
...
try {
  ...
  const first = applyBuddhismCanonBatch(data, "2026-07-20T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyBuddhismCanonBatch(data, "2026-07-20T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 174, relations: 632, timelineEvents: 45 });
  ...
  assert.equal(fullAudit.ok, true, `佛教经藏批次重复导入后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
