...
const { applyZhenlingThirdRankBatch, auditBatch } = require("./bootstrap-chinese-mythology-zhenling-third-rank.cjs");
...
const relationEventMarker = ":mythology:zhenling-third-rank:";
...
try {
  ...
  const first = applyZhenlingThirdRankBatch(data, now);
  const firstCounts = counts(data, first.worldId);
  const second = applyZhenlingThirdRankBatch(data, now);
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 76, relations: 189, timelineEvents: 6 });
  ...
  assert.equal(audit.preservedBoundaries.reusedCrossTraditionIdentities, 11);
  assert.equal(audit.preservedBoundaries.disputedPuyiRelations, 1);
