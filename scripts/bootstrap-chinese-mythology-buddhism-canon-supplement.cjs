...
const { applyBuddhismCanonBatch } = require("./bootstrap-chinese-mythology-buddhism-canon.cjs");
const {
  BATCH_LABEL,
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-canon-supplement-25.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-buddhism-canon-supplement-25-${now.replace(/[:.]/g, "-")}.wcodex`);
...
function applyBuddhismCanonSupplementBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyBuddhismCanonBatch(data, now);
  const batch = buildBuddhismCanonSupplementBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildBuddhismCanonSupplementBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    counts: {
      ...
      sourceRelations: relations.filter((item) => item.id.includes(":source-family-")).length,
      memberships: relations.filter((item) => item.label === "列入佛教文献补充目录").length,
    ...
    preservedBoundaries: {
      biographyId: supplementEntityId("family-biography-avadana", worldId),
      chanId: supplementEntityId("family-chan-records", worldId),
      lotusRecordId: supplementSourceId("jingde-chuandeng-lu", worldId),
      pureLandCommentaryId: supplementSourceId("guan-jing-four-fascicle", worldId),
      historyId: supplementSourceId("fayuan-zhulin", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyBuddhismCanonSupplementBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-canon-supplement-25");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.counts, { entities: 54, systems: 6, sources: 48, relations: 200, sourceRelations: 48, memberships: 48, timelineEvents: 18 });
  assert.deepEqual(batchAudit.trackCounts, { "textual-evidence": 10, "religious-institutions": 5, "cult-evolution": 3 });
  ...
  const report = {
    ...
    saveStats,
    batchAudit,
    buddhismDirectTotals: { entities: 637, identities: 226, relations: 2001, timelineEvents: 196, sourceEntries: 314 },
...
module.exports = { applyBuddhismCanonSupplementBatch, auditBatch };
