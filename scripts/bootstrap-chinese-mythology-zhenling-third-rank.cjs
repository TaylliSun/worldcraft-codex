...
const { applyZhenlingSecondRankBatch } = require("./bootstrap-chinese-mythology-zhenling-second-rank.cjs");
...
} = require("./chinese-mythology-zhenling-third-rank-data.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-zhenling-third-rank-14.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-zhenling-third-rank-14-${timestampForFile(now)}.wcodex`);
...
function applyZhenlingThirdRankBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyZhenlingSecondRankBatch(data, now);
  const batch = buildZhenlingThirdRankBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildZhenlingThirdRankBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    ...
    preservedBoundaries: {
      sectionCounts: expected.catalogPositions.reduce((counts, item) => {
        counts[item.section] = (counts[item.section] || 0) + 1;
        return counts;
      }, {}),
      reusedCrossTraditionIdentities: expected.catalogPositions.filter((item) => item.existingRef).length,
      disputedPuyiRelations: relations.filter((item) => item.id.endsWith(":puyi-beiyi-disputed") && item.kind === "disputed").length,
      sourceEntryId: zhenlingSourceId(worldId),
      tierEntryId: zhenlingThirdEntityId("third-rank-taiji", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyZhenlingThirdRankBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-zhenling-third-rank-14");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.preservedBoundaries, {
    sectionCounts: { "中位": 1, "左位": 51, "右位": 34 },
    reusedCrossTraditionIdentities: 11,
    disputedPuyiRelations: 1,
    sourceEntryId: zhenlingSourceId(worldId),
    tierEntryId: zhenlingThirdEntityId("third-rank-taiji", worldId)
  });
...
module.exports = { applyZhenlingThirdRankBatch, auditBatch };
