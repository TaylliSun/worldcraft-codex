...
const batch = buildConfucianRitesBatch(now, foundation.world.id);
...
assert.equal(batch.sources.length, 14);
...
assert.equal(sourceRelations.length, 46);
...
const zisiId = batch.figures.find((item) => item.title === "子思（孔伋）").id;
const menciusId = batch.figures.find((item) => item.title === "孟子").id;
const zisiMencius = batch.relations.find((item) => item.sourceEntityId === zisiId && item.targetEntityId === menciusId);
assert.ok(zisiMencius, "子思至孟子关系存在");
assert.notEqual(zisiMencius.kind, "teacher", "子思至孟子不伪造直接师承");
assert.equal(zisiMencius.confidence, "probable");
...
assert.deepEqual(
  ...
  { entities: 591, relations: 1686, timelineEvents: 229 }
);

console.log("Chinese mythology Confucian rites checks passed: 60 entries, 250 sourced relations, 35 four-track events; cumulative totals 591/1686/229.");
