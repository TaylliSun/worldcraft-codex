...
const reportPath = path.join(validationDir, "chinese-mythology-zhenling-second-rank-13.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-zhenling-second-rank-13-${timestampForFile(now)}.wcodex`
  );
...
function applyZhenlingSecondRankBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyZhenlingFirstRankBatch(data, now);
...
function auditBatch(data, worldId) {
  const expected = buildZhenlingSecondRankBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    ...
    preservedBoundaries: {
      ...
      institutionGroups: expected.institutions.filter((item) => item.title.includes("宫官")).length,
      sacredArchitecture: expected.locations.length,
      disputedRedPineRelations: relations.filter((item) => item.id.endsWith(":chisongzi-yushi-disputed") && item.kind === "disputed").length,
      sourceEntryId: zhenlingSourceId(worldId),
      tierEntryId: zhenlingSecondEntityId("second-rank-shangqing", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyZhenlingSecondRankBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-zhenling-second-rank-13");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.preservedBoundaries, {
    sectionCounts: { "中位": 1, "左位": 33, "右位": 8, "女真位": 39, "太帝宫官": 2, "金阙宫官": 10 },
    institutionGroups: 2,
    sacredArchitecture: 10,
...
module.exports = { applyZhenlingSecondRankBatch, auditBatch };
