...
const { applyZhenlingSeventhRankBatch } = require("./bootstrap-chinese-mythology-zhenling-seventh-rank.cjs");
const { celestialEntityId, celestialSourceId } = require("./chinese-mythology-celestial-bureaucracy-data.cjs");
const { BATCH_LABEL, buildThunderOfficesBatch, thunderEntityId } = require("./chinese-mythology-thunder-offices-data.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-thunder-offices-19.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-thunder-offices-19-${now.replace(/[:.]/g, "-")}.wcodex`);
...
function applyThunderOfficesBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyZhenlingSeventhRankBatch(data, now);
  const batch = buildThunderOfficesBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildThunderOfficesBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    ...
    preservedBoundaries: {
      highDivineIdentities: expected.figures.length,
      systemAndOfficeClusters: expected.institutions.length,
      ritualRecordRelations: relations.filter((item) => item.evidenceType === "ritual-record").length,
      sourceEntryId: celestialSourceId("daofa-huiyuan", worldId),
      thunderDepartmentId: celestialEntityId("thunder-department", worldId),
      highRegisterId: thunderEntityId("high-divine-register", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyThunderOfficesBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-thunder-offices-19");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.counts, { entities: 82, figures: 64, institutions: 18, relations: 256, sourceRelations: 82, containsRelations: 171, timelineEvents: 18 });
  assert.deepEqual(batchAudit.preservedBoundaries, { highDivineIdentities: 64, systemAndOfficeClusters: 18, ritualRecordRelations: 1, sourceEntryId: celestialSourceId("daofa-huiyuan", worldId), thunderDepartmentId: celestialEntityId("thunder-department", worldId), highRegisterId: thunderEntityId("high-divine-register", worldId) });
...
module.exports = { applyThunderOfficesBatch, auditBatch };
