...
const { applyBuddhismHanPeopleBatch, auditBatch } = require("./bootstrap-chinese-mythology-buddhism-han-people.cjs");
...
try {
  ...
  const first = applyBuddhismHanPeopleBatch(data, "2026-07-20T00:00:00.000Z"); const firstCounts = counts(data, first.worldId);
  const second = applyBuddhismHanPeopleBatch(data, "2026-07-20T00:00:00.000Z"); const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 156, relations: 600, timelineEvents: 40 }); assert.deepEqual(secondCounts, firstCounts);
  const batchAudit = auditBatch(data, second.worldId); for (const [key, issues] of Object.entries(batchAudit)) if (Array.isArray(issues)) assert.deepEqual(issues, [], key);
  const fullAudit = auditFullLibrary(data, store.diagnostics()); assert.equal(fullAudit.ok, true, `汉传人物批重复导入后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
