...
const { applyBuddhismDevotionBatch } = require("./bootstrap-chinese-mythology-buddhism-devotion.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-schools-09.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-buddhism-schools-09-${timestampForFile(now)}.wcodex`
  );
...
function applyBuddhismSchoolsBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyBuddhismDevotionBatch(data, now);
  const batches = buildAllBatches(now, worldId);
  const batch = batches.buddhismSchools;
  ...
  world.wiki = {
    ...
    featuredEntityIds: unique([
      ...
      ...userFeatured,
      ...batches.buddhismDevotion.featuredEntityIds,
      ...batches.buddhismTransmission.featuredEntityIds
    ]).slice(0, 12)
  };
...
function auditBuddhismSchoolsBatch(data, worldId) {
  ...
  const managedEventIds = new Set(Object.values(batches)
    ...
    .map((item) => item.id));
  const phase3Batches = [batches.buddhismTransmission, batches.buddhismDevotion, expected];
  const phase3EntityIds = new Set(phase3Batches.flatMap((item) => item.entities).map((item) => item.id));
  const phase3RelationIds = new Set(phase3Batches.flatMap((item) => item.relations).map((item) => item.id));
  const phase3EventIds = new Set(phase3Batches.flatMap((item) => item.timelineEvents).map((item) => item.id));
  ...
  return {
    ...
    phase3ManagedCounts: {
      entities: data.entities.filter((item) => phase3EntityIds.has(item.id)).length,
      relations: data.relations.filter((item) => phase3RelationIds.has(item.id)).length,
      timelineEvents: data.timelineEvents.filter((item) => phase3EventIds.has(item.id)).length
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyBuddhismSchoolsBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-schools-09");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditBuddhismSchoolsBatch(persisted, worldId);
  ...
  assert.deepEqual(audit.batchManagedCounts, { entities: 60, relations: 165, timelineEvents: 20 });
  assert.deepEqual(audit.phase3ManagedCounts, { entities: 180, relations: 500, timelineEvents: 60 });
  assert.deepEqual(audit.cumulativeManagedCounts, { entities: 531, relations: 1436, timelineEvents: 194 });
  ...
  const report = {
    ...
    phase: "中国上古神话史 · 阶段 3 · 汉传宗派、祖师谱系与核心论疏第三批",
...
module.exports = { applyBuddhismSchoolsBatch, auditBuddhismSchoolsBatch };
