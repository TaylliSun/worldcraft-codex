...
const { applyBuddhismCanonSupplementBatch } = require("./bootstrap-chinese-mythology-buddhism-canon-supplement.cjs");
const { BATCH_LABEL, buildBuddhismHanPeopleBatch, hanEntityId } = require("./chinese-mythology-buddhism-han-people-data.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-han-people-26.json");
...
function ensureCollections(data) { for (const collection of WORKSPACE_COLLECTIONS) if (!Array.isArray(data[collection])) data[collection] = []; }

async function createCompleteBackup(data, now) {
  fs.mkdirSync(backupDir, { recursive: true });
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-buddhism-han-people-26-${now.replace(/[:.]/g, "-")}.wcodex`);
...
function applyBuddhismHanPeopleBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyBuddhismCanonSupplementBatch(data, now);
  const batch = buildBuddhismHanPeopleBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildBuddhismHanPeopleBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    counts: { entities: entities.length, figures: entities.filter((item) => item.templateId?.endsWith(":deity-person")).length, systems: entities.filter((item) => item.templateId?.endsWith(":institution-ritual")).length, relations: relations.length, sourceRelations: relations.filter((item) => item.id.includes(":source-figure-")).length, memberships: relations.filter((item) => item.label === "列入汉传人物分区").length, sequence: relations.filter((item) => item.label === "本组下一人物").length, timelineEvents: timelineEvents.length },
    trackCounts: timelineEvents.reduce((counts, item) => { const key = item.trackId.split(":").at(-1); counts[key] = (counts[key] || 0) + 1; return counts; }, {}),
    preservedBoundaries: { daoanId: hanEntityId("daoan", worldId), jingyingHuiyuanId: hanEntityId("huiyuan-jingying", worldId), linjiId: hanEntityId("linji-yixuan", worldId), firstNunId: hanEntityId("jingjian-nun", worldId), modernEducationId: hanEntityId("taixu", worldId) },
...
async function main() {
  ...
  try {
    const loaded = store.load(); const now = new Date().toISOString();
    backup = await createCompleteBackup(loaded.data, now);
    ({ worldId } = applyBuddhismHanPeopleBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-han-people-26");
  } finally { store.close(); }
  const verifyStore = new WorkspaceStore({ dbPath, backupDir, schemaVersion: 17 });
  let batchAudit; let fullAudit; let diagnostics;
  try { const persisted = verifyStore.load().data; diagnostics = verifyStore.diagnostics(); batchAudit = auditBatch(persisted, worldId); fullAudit = auditFullLibrary(persisted, diagnostics); } finally { verifyStore.close(); }
  assert.deepEqual(batchAudit.counts, { entities: 156, figures: 150, systems: 6, relations: 600, sourceRelations: 140, memberships: 150, sequence: 144, timelineEvents: 40 });
  assert.deepEqual(batchAudit.trackCounts, { "textual-evidence": 20, "religious-institutions": 10, "cult-evolution": 10 });
  for (const [key, issues] of Object.entries(batchAudit)) if (Array.isArray(issues)) assert.deepEqual(issues, [], `${key}: ${issues.join(", ")}`);
  assert.equal(diagnostics.ok, true); assert.equal(fullAudit.ok, true, `全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
  const report = { ok: true, generatedAt: new Date().toISOString(), phase: BATCH_LABEL, backup: backup.targetPath, saveStats, batchAudit, buddhismDirectTotals: { entities: 793, identities: 376, relations: 2601, timelineEvents: 236, sourceEntries: 314 }, fullLibrary: { counts: fullAudit.counts, trackCounts: fullAudit.trackCounts, benchmarks: fullAudit.benchmarks }, diagnostics: { ok: diagnostics.ok, schemaVersion: diagnostics.schemaVersion, quickCheck: diagnostics.quickCheck, foreignKeyIssues: diagnostics.foreignKeyIssues, dbPath: diagnostics.dbPath } };
  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  console.log(JSON.stringify({ ok: true, reportPath, ...report }, null, 2));
}

if (require.main === module) main().catch((error) => { console.error(error?.stack || error); process.exitCode = 1; });
module.exports = { applyBuddhismHanPeopleBatch, auditBatch };
