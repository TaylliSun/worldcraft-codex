...
const { buildThunderOfficesBatch, SOURCE_CITATION, thunderEntityId } = require("./chinese-mythology-thunder-offices-data.cjs");
...
assert.deepEqual({ entities: batch.entities.length, figures: batch.figures.length, institutions: batch.institutions.length, relations: batch.relations.length, events: batch.timelineEvents.length }, { entities: 82, figures: 64, institutions: 18, relations: 256, events: 18 });
...
assert.equal(batch.entities.some((item) => item.id === thunderEntityId("high-divine-register", worldId)), true);
assert.equal(SOURCE_CITATION, "《道法会元》卷五十六〈上清玉府大法〉“雷霆分司”");
...
assert.equal(batch.relations.filter((item) => item.id.includes(":source-")).length, 82);
assert.equal(batch.relations.filter((item) => item.kind === "contains").length, 171);
assert.equal(batch.relations.filter((item) => item.evidenceType === "ritual-record").length, 1);
...
assert.deepEqual(batch.timelineEvents.reduce((counts, item) => {
  ...
  return counts;
}, {}), { "religious-institutions": 15, "textual-evidence": 3 });
...
}, { entities: 1457, relations: 4148, timelineEvents: 327 });

console.log("Chinese mythology thunder-office checks passed: 64 high divine identities, 18 institutions and office clusters, 82 entries, 256 relations, 18 events; cumulative totals 1457/4148/327.");
