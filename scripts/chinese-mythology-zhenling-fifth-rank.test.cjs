...
const { buildZhenlingThirdRankBatch, zhenlingThirdEntityId } = require("./chinese-mythology-zhenling-third-rank-data.cjs");
const { buildZhenlingFourthRankBatch } = require("./chinese-mythology-zhenling-fourth-rank-data.cjs");
const { buildZhenlingFifthRankBatch, zhenlingFifthEntityId } = require("./chinese-mythology-zhenling-fifth-rank-data.cjs");
...
}, { entities: 47, figures: 33, institutions: 6, locations: 8, sources: 0, positions: 37, reused: 1, offices: 3, relations: 136, events: 6 });
...
assert.deepEqual(batch.catalogPositions.filter((item) => item.existingRef).map((item) => item.existingRef), ["z3:zhang-feng"]);
assert.equal(batch.figures.some((item) => item.title === "张奉"), false);
assert.equal(batch.entities.some((item) => item.id === zhenlingFifthEntityId("fifth-rank-nine-offices", worldId)), true);
assert.equal(batch.entities.some((item) => item.id === zhenlingSourceId(worldId)), false);
assert.equal(batch.catalogPositions[0].existingRef, "z3:zhang-feng");
...
for (const figure of batch.figures) {
  ...
  assert.equal(figure.templateData.rankPosition, "第五阶 · 九宫诸曹");
}
...
assert.equal(batch.relations.filter((item) => item.id.includes(":source-")).length, 48);
assert.equal(batch.relations.filter((item) => item.kind === "contains").length, 72);
assert.equal(batch.relations.filter((item) => item.kind === "disputed").length, 2);
assert.equal(batch.relations.filter((item) => item.kind === "located").length, 10);
assert.equal(batch.relations.filter((item) => item.kind === "teacher").length, 3);
...
assert.deepEqual(batch.timelineEvents.reduce((counts, item) => {
  ...
  return counts;
}, {}), { "textual-evidence": 2, "religious-institutions": 2, "cult-evolution": 2 });
...
}, { entities: 1078, relations: 2975, timelineEvents: 292 });

console.log("Chinese mythology Zhenling fifth-rank checks passed: 37 positions, 1 reused identity, 33 new figures, 3 unnamed offices, 47 entries, 136 relations, 6 events; cumulative totals 1078/2975/292.");
