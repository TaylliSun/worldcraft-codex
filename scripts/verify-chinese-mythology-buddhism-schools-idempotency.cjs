...
const {
  applyBuddhismSchoolsBatch,
  auditBuddhismSchoolsBatch
} = require("./bootstrap-chinese-mythology-buddhism-schools.cjs");
...
const marker = ":mythology:buddhism-schools:";
...
try {
  ...
  const first = applyBuddhismSchoolsBatch(data, now);
  const firstCounts = counts(data, first.worldId);
  const second = applyBuddhismSchoolsBatch(data, now);
  ...
  const audit = auditBuddhismSchoolsBatch(data, second.worldId);
