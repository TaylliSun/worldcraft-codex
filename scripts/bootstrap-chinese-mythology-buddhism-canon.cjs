...
const { applyBuddhismPantheonBatch } = require("./bootstrap-chinese-mythology-buddhism-pantheon.cjs");
const {
  BATCH_LABEL,
...
const reportPath = path.join(validationDir, "chinese-mythology-buddhism-canon-core-24.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-buddhism-canon-core-24-${now.replace(/[:.]/g, "-")}.wcodex`
  );
...
function applyBuddhismCanonBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyBuddhismPantheonBatch(data, now);
  const batch = buildBuddhismCanonBatch(now, worldId);
...
function auditBatch(data, worldId) {
  const expected = buildBuddhismCanonBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    counts: {
      ...
      familyMemberships: relations.filter((item) => item.label === "列入本批经律论家族").length,
      readingSequence: relations.filter((item) => item.label === "同家族下一阅读入口").length,
      familyComparisons: relations.filter((item) => item.label === "同一文献家族对读").length,
    ...
    preservedBoundaries: {
      agamaFamilyId: canonEntityId("family-agama", worldId),
      vinayaFamilyId: canonEntityId("family-vinaya", worldId),
      surangamaId: canonSourceId("great-surangama", worldId),
      perfectEnlightenmentId: canonSourceId("perfect-enlightenment", worldId),
      northernDipperId: canonSourceId("northern-dipper-life", worldId),
      catalogueId: canonSourceId("kaiyuan-catalogue", worldId)
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyBuddhismCanonBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-buddhism-canon-core-24");
  } finally {
    store.close();
  }
...
module.exports = { applyBuddhismCanonBatch, auditBatch };
