...
const { buildDaoismLineagesBatch, lineageEntityId } = require("./chinese-mythology-daoism-lineages-data.cjs");
...
const previousBatches = [
  ...
  buildThunderOfficesBatch(now, worldId), buildFengduOfficesBatch(now, worldId)
];
...
const sourceIds = new Set(allEntities.filter((item) => item.templateId?.endsWith(":source-text")).map((item) => item.id));
...
}, { entities: 44, figures: 17, institutions: 24, sources: 3, relations: 134, events: 26 });
...
assert.equal(SOURCE_CITATION, "《上清六甲祈祷秘法》《灵宝六丁秘法》及《道法会元》卷一百七十五");
assert.equal(batch.entities.some((item) => item.id === dingjiaEntityId("dingjia-system", worldId)), true);
assert.equal(batch.entities.some((item) => item.id === dingjiaSourceId("lingbao-liuding", worldId)), true);
assert.equal(batch.entities.some((item) => item.title === "王灵官（隆恩真君）"), false, "王灵官必须复用既有身份");
assert.equal(batch.entities.some((item) => item.title === "玉皇上帝"), false, "玉皇上帝必须复用既有身份");
assert.equal(batch.relations.some((item) => item.sourceEntityId === dingjiaEntityId("fire-temple-lingguan-hall", worldId) && item.targetEntityId === lineageEntityId("wang-lingguan", worldId)), true);
assert.equal(batch.relations.some((item) => item.sourceEntityId === dingjiaEntityId("taifu-yuhuang-hall", worldId) && item.targetEntityId === celestialEntityId("jade-emperor", worldId)), true);
assert.equal(batch.relations.some((item) => item.sourceEntityId === dingjiaEntityId("liuding-yin-register", worldId) && item.targetEntityId === zhenlingFourthEntityId("liuding-shizhe", worldId)), true);
...
for (const relation of batch.relations) {
  ...
  if (relation.id.includes(":source-")) {
    assert.equal(relation.strength, 5, `${relation.id}: source strength`);
    assert.ok(sourceIds.has(relation.targetEntityId), `${relation.id}: source target`);
  }
}
assert.equal(batch.relations.filter((item) => item.id.includes(":source-")).length, 52);
assert.equal(batch.relations.filter((item) => item.kind === "contains").length, 55);
assert.equal(batch.relations.filter((item) => item.evidenceType === "material-evidence").length, 18);
...
assert.deepEqual(batch.timelineEvents.reduce((counts, item) => {
  ...
  return counts;
}, {}), { "textual-evidence": 7, "religious-institutions": 17, "cult-evolution": 2 });
...
}, { entities: 1595, relations: 4572, timelineEvents: 397 });

console.log("Chinese mythology Dingjia/guardian checks passed: 17 figures, 24 systems and temple entries, 3 sources, 134 relations, 26 events; cumulative totals 1595/4572/397.");
