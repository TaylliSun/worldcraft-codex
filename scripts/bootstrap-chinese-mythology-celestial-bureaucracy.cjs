...
const { applyDaoismEarlyBatch } = require("./bootstrap-chinese-mythology-daoism-early.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-celestial-bureaucracy-05.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-celestial-bureaucracy-05-${timestampForFile(now)}.wcodex`
  );
...
function applyCelestialBureaucracyBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyDaoismEarlyBatch(data, now);
  ...
  const batch = buildCelestialBureaucracyBatch(now, worldId);
...
function auditCelestialBureaucracyBatch(data, worldId) {
  ...
  const expected = buildCelestialBureaucracyBatch(auditTime, worldId);
  ...
  const sourceRelationIssues = relations
    ...
    .filter((item) => item.strength !== 5 || !expectedSourceIds.has(item.targetEntityId))
    .map((item) => item.id);
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyCelestialBureaucracyBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-celestial-bureaucracy-05");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditCelestialBureaucracyBatch(persisted, worldId);
  ...
  assert.deepEqual(audit.cumulativeManagedCounts, { entities: 301, relations: 756, timelineEvents: 114 });
  ...
  const report = {
    ...
    phase: "中国上古神话史 · 阶段 2 · 星辰、三官五岳与天界官署第二批",
...
module.exports = { applyCelestialBureaucracyBatch, auditCelestialBureaucracyBatch };
