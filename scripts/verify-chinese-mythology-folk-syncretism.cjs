...
const reportPath = path.join(validationDir, "chinese-mythology-folk-syncretism-ui.json");
const wikiScreenshotPath = path.join(validationDir, "chinese-mythology-folk-syncretism-wiki.png");
const relationsScreenshotPath = path.join(validationDir, "chinese-mythology-folk-syncretism-relations.png");
const timelineScreenshotPath = path.join(validationDir, "chinese-mythology-folk-syncretism-timeline.png");
...
async function run() {
  ...
  try {
    ...
    const dataAudit = await page.evaluate(async (expectedName) => {
      ...
      const batchMarker = ":mythology:folk-syncretism:";
      ...
      const managedRelations = data.relations.filter(isManagedRelationOrEvent);
      const managedEvents = data.timelineEvents.filter(isManagedRelationOrEvent);
      return {
        worldId: world.id,
        batch: {
    ...
    assert.deepEqual(dataAudit.cumulativeManaged, { entities: 651, relations: 1936, events: 264 });
    ...
    await page.locator(".wiki-stat-band").getByText("651 篇文章", { exact: true }).waitFor();
    await page.locator(".wiki-stat-band").getByText("264 个历史节点", { exact: true }).waitFor();

    const wikiSearch = page.getByLabel("搜索世界 Wiki");
    await wikiSearch.fill("妈祖");
    const result = exactWikiResult(page, "妈祖（天妃、天后）");
    await result.waitFor({ state: "visible", timeout: 60000 });
    await result.click();
    await page.locator(".wiki-article h1").getByText("妈祖（天妃、天后）", { exact: true }).waitFor();
    await page.locator(".wiki-rich-content").getByText("妈祖的故事从海边庙宇出发", { exact: false }).waitFor();
    await page.locator(".wiki-rich-content").getByText("地方灵验纳入海运秩序", { exact: false }).waitFor();
    ...
    await page.locator(".relation-list-panel").getByText("1936 条符合筛选", { exact: true }).waitFor();
    await page.getByLabel("搜索关系").fill("赵公明");
    ...
    assert.match(await page.getByLabel("关系原典出处").inputValue(), /三教源流搜神大全/u);
    assert.match(await page.getByLabel("关系适用年代").inputValue(), /明代/u);
    ...
    assert.match((await trackButtons.filter({ hasText: "文献证据" }).textContent()) || "", /83 项/u);
    assert.match((await trackButtons.filter({ hasText: "宗教与礼制制度" }).textContent()) || "", /67 项/u);
    assert.match((await trackButtons.filter({ hasText: "信仰演变与合流" }).textContent()) || "", /44 项/u);
    await trackButtons.filter({ hasText: "宗教与礼制制度" }).click();
    await page.locator(".timeline-track-board").getByText("元代加封梓潼神为文昌帝君", { exact: true }).waitFor();
    await page.locator(".timeline-track-board").getByText("清初天后封号扩大沿海国家祭祀", { exact: true }).waitFor();
    ...
    const report = {
      ...
      dataAudit,
      ui: {
        wikiCountsVisible: { articles: 651, timelineEvents: 264 },
        openedArticle: "妈祖（天妃、天后）",
        evidenceRelationVisible: "神谱中奉命任玄坛元帅",
        timelineCountsVisible: { mythic: 70, textual: 83, institutional: 67, evolution: 44 }
      },
