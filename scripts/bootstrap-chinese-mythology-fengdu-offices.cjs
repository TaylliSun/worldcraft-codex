...
const { applyThunderOfficesBatch } = require("./bootstrap-chinese-mythology-thunder-offices.cjs");
const { celestialEntityId, celestialSourceId } = require("./chinese-mythology-celestial-bureaucracy-data.cjs");
const { BATCH_LABEL, buildFengduOfficesBatch, fengduEntityId } = require("./chinese-mythology-fengdu-offices-data.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-fengdu-offices-20.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(backupDir, `worldcraft-codex-complete-before-fengdu-offices-20-${now.replace(/[:.]/g, "-")}.wcodex`);
...
function applyFengduOfficesBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyThunderOfficesBatch(data, now);
...
function auditBatch(data, worldId) {
  const expected = buildFengduOfficesBatch("2000-01-01T00:00:00.000Z", worldId);
  ...
  return {
    ...
    preservedBoundaries: {
      namedFigures: expected.figures.length,
      palaceAndOfficeEntries: expected.institutions.length,
      namedPalaces: expected.institutions.filter((item) => item.templateData?.institutionKind === "冥府宫城" && item.id !== fengduEntityId("twenty-four-palaces", worldId)).length,
      ...
      fengduEmperorId: celestialEntityId("fengdu-emperor", worldId),
      reusedGuandiId: folkEntityId("guan-di", worldId),
      rootSystemId: fengduEntityId("fengdu-examination-system", worldId),
      incompleteNameIds: [fengduEntityId("incomplete-wang-general", worldId), fengduEntityId("partial-prison-masters", worldId)]
    },
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyFengduOfficesBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-fengdu-offices-20");
  } finally {
    store.close();
  }
  ...
  assert.deepEqual(batchAudit.counts, { entities: 94, figures: 58, institutions: 36, relations: 290, sourceRelations: 94, containsRelations: 187, timelineEvents: 44 });
  assert.deepEqual(batchAudit.preservedBoundaries, {
    namedFigures: 58,
    palaceAndOfficeEntries: 36,
    namedPalaces: 12,
    ritualRecordRelations: 2,
    sourceEntryId: celestialSourceId("daofa-huiyuan", worldId),
    fengduEmperorId: celestialEntityId("fengdu-emperor", worldId),
    reusedGuandiId: folkEntityId("guan-di", worldId),
    rootSystemId: fengduEntityId("fengdu-examination-system", worldId),
...
module.exports = { applyFengduOfficesBatch, auditBatch };
