...
const { applyConfucianRitesBatch } = require("./bootstrap-chinese-mythology-confucian-rites.cjs");
...
const reportPath = path.join(validationDir, "chinese-mythology-folk-syncretism-11.json");
...
async function createCompleteBackup(data, now) {
  ...
  const targetPath = path.join(
    backupDir,
    `worldcraft-codex-complete-before-mythology-folk-syncretism-11-${timestampForFile(now)}.wcodex`
  );
...
function buildAllBatches(now, worldId) {
  return {
    ...
    folkSyncretism: buildFolkSyncretismBatch(now, worldId)
  };
}

function applyFolkSyncretismBatch(data, now) {
  ensureCollections(data);
  const { worldId } = applyConfucianRitesBatch(data, now);
  const batches = buildAllBatches(now, worldId);
  const batch = batches.folkSyncretism;
  ...
  world.wiki = {
    ...
    featuredEntityIds: unique([
      ...
      ...userFeatured,
      ...batches.confucianRites.featuredEntityIds
    ]).slice(0, 12)
  };
...
function auditFolkSyncretismBatch(data, worldId) {
  ...
  const sourceCoverageIssues = entities
    .filter((item) => !item.content.includes("<h2>资料边界</h2>"))
    .map((item) => item.title);
...
async function main() {
  ...
  try {
    ...
    ({ worldId } = applyFolkSyncretismBatch(loaded.data, now));
    saveStats = store.save(loaded.data, "bootstrap-chinese-mythology-folk-syncretism-11");
  } finally {
    store.close();
  }
  ...
  try {
    ...
    audit = auditFolkSyncretismBatch(persisted, worldId);
  ...
  assert.deepEqual(audit.cumulativeManagedCounts, { entities: 651, relations: 1936, timelineEvents: 264 });
  ...
  const report = {
    ...
    phase: "中国上古神话史 · 阶段 4 · 地方神职、行业信仰与三教合流第二批",
...
module.exports = { applyFolkSyncretismBatch, auditFolkSyncretismBatch };
