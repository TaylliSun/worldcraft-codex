...
const { applyBuddhismTibetanBatch, auditBatch } = require("./bootstrap-chinese-mythology-buddhism-tibetan.cjs");
...
try {
  ...
  const first = applyBuddhismTibetanBatch(data, "2026-07-20T00:00:00.000Z");
  const firstCounts = counts(data, first.worldId);
  const second = applyBuddhismTibetanBatch(data, "2026-07-20T00:00:00.000Z");
  const secondCounts = counts(data, second.worldId);
  assert.deepEqual(firstCounts, { entities: 106, relations: 340, timelineEvents: 30 });
  ...
  assert.equal(fullAudit.ok, true, `藏传佛教批重复导入后的全库审计未通过：${JSON.stringify(fullAudit.issues)}`);
